...
function handleRegister(event) {
    ...
    if (allUsers.some(u => u.username === username)) {
        showErrorMessage('Это имя пользователя уже занято. Выберите другое.');
        return;
    }
    ...
    // Очищаем форму и скрываем ошибку
    clearFormFields();
    hideErrorMessage();
    
    // Показываем уведомление об успешной регистрации
    alert(`Добро пожаловать в Astrales, ${username}! 🎉`);
}
...
function handleLogout() {
    // НЕ удаляем пользователя из localStorage - он остается в системе
    // Просто скрываем меню профиля
...
function saveProfileChanges() {
    ...
    // Всегда сохраняем изменения (даже если ничего не изменилось)
